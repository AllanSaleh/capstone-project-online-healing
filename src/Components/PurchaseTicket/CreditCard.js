...
export default function CreditCard({ background, logo, date, number, name }) {
  ...
  return (
    <div className="relative border-2 border-red-600 inline-block w-2/6 h-80 box-border mx-12">
      <img className="absolute w-full h-full" src={backgrounds} alt="Background" />
      <img className="absolute top-10 right-6" src={logos} alt="Logo" />
      <div className="absolute top-14 left-4 text-white text-paragraph">{date}</div>
      <div className="absolute top-28 left-4 text-white text-paragraph">{number}</div>
      <div className="absolute top-40 left-4 text-white text-paragraph">{name}</div>
      <button
        type="button"
